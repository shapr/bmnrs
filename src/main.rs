...
fn play_many(cards: Vec<u8>) {
    ...
    while let Some(mut gamestate) = games_best.pop() {
        ...
        let mut wiggled = wiggle_all(&mut gamestate);
        while let Some(mut wiggled_game) = wiggled.pop() {
...
fn wiggle_all(g: &mut GameState) -> Vec<GameState> {
    ...
    for c in 1..4 {
        ...
        // println!("wiggled_this_card is size {}", wiggled_this_card.len());
        wiggled_games.append(&mut wiggled_this_card);
    }
    wiggled_games.sort_unstable();
    wiggled_games.dedup();
    // println!("wiggled_games is size {}", wiggled_games.len());
...
fn wiggle(g: &mut GameState, penalty_card: u8) -> Vec<GameState> {
    ...
    let not_this_penalty_card_indices = cards
        .iter()
        .enumerate()
        .filter_map(|(index, &c)| if c != penalty_card { Some(index) } else { None })
        .collect::<Vec<_>>();
    let mut gvec: Vec<GameState> = Vec::with_capacity(64);
    if let Some(ace_index) = maybe_this_penalty_card_index {
        let mut not_this_card_indices = not_this_penalty_card_indices.clone();
        while let Some(not_ace_index) = not_this_card_indices.pop() {
...
fn good_games() -> Vec<GameState> {
    let games = [
        ("---JQ---K-A----A-J-K---QK-", "-J-----------AJQA----K---Q"), // smallest record
        ("-J--KA----A-Q--Q-A----KJ--", "A------QKJ--Q-------KJ----"), // second smallest record
        ("K-KK----K-A-----JAA--Q--J-", "---Q---Q-J-----J------AQ--"), // third smallest
        ("----Q------A--K--A-A--QJK-", "-Q--J--J---QK---K----JA---"), // 4th smallest
        ("---AK-Q--J----J--QKJ-Q----", "------JK-----A--K--Q---AA-"), // etc
        ("--A-Q--J--J---Q--AJ-K---K-", "-J-------Q------A--A--QKK-"), // etc
        ("A-AQ-----Q--K--AQ-------JJ", "-J-A-KKJ--K-----------Q---"), // etc
        ("-AJ--QK--K----Q--J-A-KKJ--", "---------JQ----------A-AQ-"), // etc
        ("-J------Q------AAA-----QQ-", "K----JA-----------KQ-K-JJK"), // etc
        ("K----JA-----------KQ-K-JJK", "-----KK---------A-JK-Q-Q-Q"), // etc
        ("-----KK---------A-JK-Q-Q-Q", "-----KK---------A-JK-Q-Q-Q"), // etc
