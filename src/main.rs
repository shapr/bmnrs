...
// pub use crate::tests; XXX how does this work?
...
fn main() {
    ...
    let all_hands: Vec<Vec<u8>> = cards_clone
	.iter()
	.cloned()
	.permutations(cards.len())
	.unique()
	.collect();
    let finished_games: Vec<(&Vec<u8>, u16)> =
	ParallelIterator::collect(all_hands.par_iter().map(|deck| {
	    let c = deck.clone();
	    let mut gamestate = deal(c, false);

	    let steps = play_one(&mut gamestate.game);

	    return (deck, steps);
	}));
    println!("{:?}", finished_games);
...
fn record_best_play_one(
    ...
    let steps = play_one(&mut gs.game);
